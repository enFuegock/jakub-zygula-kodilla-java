...
class WeatherForecastTestSuite {
    ...
    @Test
    void testCalculateTheAvarageTemperatureWithMock(){
        //Given
        when(temperaturesMock.getTemperatures()).thenReturn(temperaturesMap);
        WeatherForecast weatherForecast = new WeatherForecast(temperaturesMock);

        //When
        double averageTemperature = weatherForecast.calculateTheAvarageTemperature();

        //Then
        Assertions.assertEquals(25.56, averageTemperature, 0.01);
    }
    @Test
    void testFindTheMedianOfTemperaturesWithMock() {
        //Given
        when(temperaturesMock.getTemperatures()).thenReturn(temperaturesMap);
        WeatherForecast weatherForecast = new WeatherForecast(temperaturesMock);

        //When
        double medianTemperature = weatherForecast.findTheMedianOfTemperatures();
